...
struct SubscriptionsMusicView: View {
    ...
    private struct Option: Identifiable, Hashable {
        let id = UUID()
        let type: QuestionnaireViewModel.SubscriptionMusicType
    ...
    private let options: [Option] = [
        Option(type: .AppleMusic,   assetName: "logo_apple_music",   fallbackSystemName: "applelogo",         remoteLogoURL: "https://logo.clearbit.com/apple.com",    title: "Apple Music"),
        Option(type: .Deezer,       assetName: "logo_deezer",        fallbackSystemName: "music.note.list",   remoteLogoURL: "https://logo.clearbit.com/deezer.com",   title: "Deezer"),
        Option(type: .Spotify,      assetName: "logo_spotify",       fallbackSystemName: "music.note",        remoteLogoURL: "https://logo.clearbit.com/spotify.com",  title: "Spotify"),
        Option(type: .AmazonMusic,  assetName: "logo_amazon_music",  fallbackSystemName: "music.note",        remoteLogoURL: "https://logo.clearbit.com/amazon.com",   title: "Amazon Music"),
        Option(type: .Other,        assetName: "logo_music_other",   fallbackSystemName: "questionmark",      remoteLogoURL: nil,                                       title: "Autre")
    ]

    private var canProceed: Bool { !vm.selectedSubscriptionsMusic.isEmpty }
    ...
    private func toggle(_ type: QuestionnaireViewModel.SubscriptionMusicType) {
        let willSelect = !vm.selectedSubscriptionsMusic.contains(type)
        let generator = UIImpactFeedbackGenerator(style: willSelect ? .medium : .light)
        generator.impactOccurred()
        withAnimation(.spring(response: 0.3, dampingFraction: 0.8)) {
            if vm.selectedSubscriptionsMusic.contains(type) {
                vm.selectedSubscriptionsMusic.remove(type)
            } else {
                vm.selectedSubscriptionsMusic.insert(type)
            }
        }
    }
...
struct SubscriptionMusicOptionCard: View {
    let assetName: String
    let fallbackSystemName: String
    let remoteLogoURL: String?
    let title: String
...
#Preview {
    NavigationStack {
        SubscriptionsMusicView()
            .environmentObject(QuestionnaireViewModel())
    }
}
//
//  SubscriptionMusicView.swift
//  Auth
//
